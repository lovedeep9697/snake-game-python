    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if ((event.key== K_LEFT or event.key==K_d)) and direction != RIGHT:
                    direction = LEFT
        ...
        # if worm hits itself or edge----game over
        ...
        # move snake in direction pressed by user and add a segment
        if direction == UP :
...
def drawPressKeyMsg():
    pressKeySurf = BASICFONT.render('press key to play',True,DARKGRAY)
...
def drawSnake(wormCoords):
    for coord in wormCoords:
        ...
        pygame.draw.rect(DISPLAYSURF,DARKGREEN,snakeinnersegRect)
